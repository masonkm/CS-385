    ...
    RedBlackTreeIterator& operator++() {
        ...
        if (node_ptr == nullptr) {
            ...
            if (node_ptr->right != nullptr) {
                ...
                // root_parent_. A non-nullptr parent is the successor. If parent
...
private:
    ...
    int height(Node<K, V> *node) const {
    		if(node == root_ && root_ == nullptr){
            	...
            	if(node == nullptr){
            		...
            		int maxh = height(node->left);
            		if(height(node->left) < height(node->right)) maxh = height(node->right);
            		return 1 + maxh;
            	}
    		}
    	}
    ...
    int diameter(Node<K, V> *node) const {
        if(node == nullptr) return 0;
        int heightsum = height(node->left) + height(node->right) + 2;
        int maxd = diameter(node->left);
        	if(diameter(node->left) < diameter(node->right)) maxd = diameter(node->right);
        if(heightsum > maxd){
        		return heightsum;
        }
        return maxd;
